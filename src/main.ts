...
function applyProfile(brightness: number, temperature: number) {
  const cmdStr = `./gnome-gamma-tool/gnome-gamma-tool.py -b ${
    brightness.toFixed(2)
  } -t ${temperature} -y`;
  print(cmdStr);
  const [success, stdout, _stderr, exit_status] = GLib
